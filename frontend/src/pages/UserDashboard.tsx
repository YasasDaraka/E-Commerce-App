...
export default function UserDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {activeTab === 'profile' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-[#EEF0F6]/60 shadow-lg rounded-lg p-6">
            ...
            <button className="mt-4 bg-[#F88655] hover:bg-orange-500 transition text-white px-4 py-1 rounded-md">Edit Address</button>
          </div>
        </div>
      )}
        ...
        <div className="bg-[#EEF0F6]/50 shadow-lg rounded-lg p-6 border-t-2 border-t-[#eef0f6f6] ">
          ...
          {orders.length > 0 ? (
            orders.map((order:any) => (
              <div key={order._id.slice(0,6)} className="mb-6 p-4 bg-[#EEF0F6]/90 border border-gray-200 rounded-lg shadow-sm mx-4">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="font-medium text-orange-600/80">Order : {order._id.slice(0,6)}</h3>
                  <span className={`py-1 px-3 rounded-full text-xs bg-orange-500/70 text-white`}>{order.status}</span>
                </div>
                <div className='flex flex-row justify-start items-center w-4/5'>
                  {order?.orderDetails.map((orderDetail:any) => (
                    <div key={orderDetail.itemId.itemName} className="flex items-center mb-2 mr-4 ">
                      <img src={orderDetail.itemId.image} alt={orderDetail.itemId.itemName} className="w-16 h-16 object-cover mr-4 rounded-md mix-blend-multiply" />
                      <div>
                        <p className="font-medium text-gray-5=600">{orderDetail.itemId.itemName}</p>
      ...
      {activeTab === 'wishlist' && (
        <div className="bg-[#EEF0F6]/50 border-t-2 border-t-[#F8F2EE]/50 shadow-lg rounded-lg p-6">
                ...
                (item.item != null ? 
                  ...
                  <div>
                    ...
                    <button 
                      ...
                      className={`mt-4 ${item.item.qty > 0 ? 'bg-[#F88655] text-white hover:bg-orange-500' : 'bg-gray-300 text-gray-500'}  px-4 py-1 rounded-3xl`}
    ...
    {activeTab === 'payments' && (
      ...
      <button className="bg-[#F88655] text-white hover:bg-orange-500  transition px-4 py-1 rounded-3xl ml-0.5">Add Payment Method</button>
    </div>
    )}
